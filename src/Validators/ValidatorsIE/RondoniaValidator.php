<?php

namespace InovantiBank\Toolkit\Validators\ValidatorsIE;

use InovantiBank\Toolkit\Interfaces\ValidadorIEInterface;

class RondoniaValidator implements ValidadorIEInterface
{
    /**
     * Verifica se a Inscrição Estadual de Rondônia é válida,
     * conforme a regra: http://www.sintegra.gov.br/Cad_Estados/cad_RO.html
     *
     * @param  string  $ie  Inscrição Estadual (apenas números)
     * @return bool Retorna true se a IE for válida, false caso contrário.
     */
    public function isValid(string $ie): bool
    {
        // A IE de Rondônia deve ter 14 dígitos.
        if (strlen($ie) !== 14) {
            return false;
        }

        return $this->calculate($ie);
    }

    /**
     * Realiza o cálculo do dígito verificador para a IE de Rondônia.
     *
     * @param  string  $ie  IE sanitizada (apenas números)
     * @return bool Retorna true se o dígito verificador calculado for igual ao décimo quarto dígito da IE.
     */
    private function calculate(string $ie): bool
    {
        $sum = 0;
        $length = strlen($ie);
        $position = $length - 1;
        $weight = 6;
        $body = substr($ie, 0, $position);

        foreach (str_split($body) as $digit) {
            $sum += $digit * $weight;
            $weight--;
            if ($weight == 1) {
                $weight = 9;
            }
        }

        $remainder = $sum % 11;
        $checkDigit = 11 - $remainder;

        if ($checkDigit >= 10) {
            $checkDigit -= 10;
        }

        return $checkDigit == (int) $ie[$position];

    }
}
